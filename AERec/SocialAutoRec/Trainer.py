...
from AutoEncoder import AutoEncoder as Model
...
class Trainer(object):
    ...
    def train(self):
        ...
        for e in range(opts.epochs):
            self.epoch += 1
            self.adjust()
            user_indices, user_masks, user_items, item_users = streamer.next_train_batch()
            model(user_indices, user_masks, user_items, item_users)
            if opts.save:
                self.val()
    ...
    def val(self):
        ...
        scores_hat = []
        scores = []
        yhat = model.test().data.cpu().numpy()
        while True:
            val_idx += 1
            ...
            for user, item in zip(users, items):
                score_hat.append(np.clip(yhat[user, item], 0, 4))
        ...
        print('[Val][MSE:{:.5f}]'.format(val_score))
