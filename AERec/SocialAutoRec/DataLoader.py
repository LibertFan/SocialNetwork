...
class DataLoader(object):
    def __init__(self, opts):
        ...
        # 使用离散的句子表示！
        # self.user_cluster = self.load_user_cluster(opts.user_cluster_path)
        # 在option当中加入前途向量的维度
    ...
    def load_user_cluster(self, user_cluster_path):
        user_cluster = np.zeros(self.user_num, self.user_num)
        with open(user_cluster_path, 'r') as f:
            lines = f.readlines()
            for line in lines:
                user, follower = [int(v)-1 for v in line.split('\t')]
                user_cluster[user, follower] = 1
        return user_cluster

    @staticmethod
    def stat_score(dat):
        # users, items = list(set(dat[:, 0])), list(set(dat[:, 1]))
        # print(max(users), min(users), max(items), min(items))
        # print('[Users:{}][Items:{}]'.format(len(users), len(items)))
        # user_num = max(users) - min(users) + 1
        # item_num = max(items) - min(items) + 1
        user_num = 1000
        item_num = 1000
        count = np.zeros([user_num, item_num]) - 1
        for record in dat:
            user_id, item_id, score = record
            count[user_id, item_id] = score
    ...
    def next_train_user_batch(self):
        ...
        if start_idx >= self.user_num:
            ...
            return user_indices, user_masks, user_items
...
def read_commands():
    ...
    parser.add_argument('--model_name', type=str, default='AEV0')
...
def main():
    ...
    for _ in range(5):
        train_dat = streamer.next_train_batch()
        print(train_dat[1][:200])
        print(train_dat[2])
        print(train_dat[3])
